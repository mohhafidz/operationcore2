...
class AutoUpdater {
  ...
  static Future<void> checkForUpdates(BuildContext context) async {
    try {
      ...
      if (response.statusCode == 200) {
        final latestVersionTag = response.data['tag_name'] as String; // Contoh: "v1.0.1"
  ...
  static void _showUpdateDialog(BuildContext context, String newVersion, String downloadUrl) {
    showDialog(
      context: context,
      barrierDismissible: false, // User tidak bisa klik luar pop-up, harus pilih opsi
      builder: (context) => _UpdateDialogUI(downloadUrl: downloadUrl, newVersion: newVersion),
    );
  }
}
...
class _UpdateDialogUIState extends State<_UpdateDialogUI> {
  ...
  Future<void> _startDownload() async {
    ...
    try {
      ...
      final savePath = "${tempDir.path}\\OperationCore-Setup-${widget.newVersion}.exe";
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Download gagal. Silakan coba lagi nanti.", style: GoogleFonts.inter())),
        );
      }
    }
  }
      ...
      actions: isDownloading
          ...
          : [
              ...
              ElevatedButton(
                ...
                child: Text("Update Sekarang", style: GoogleFonts.inter(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ],
    );
  }
